...
class ReviewController {
    ...
    @GetMapping
    fun getAll(
        @ApiParam(value = "Book id provided by google books") @PathVariable bookId: String,
        @ApiParam(value = "Page number of reviews (zero-based indexing)") @RequestParam page: Int?
    ): ReviewsResponse =
        ReviewsResponse(
            3,
    ...
    fun getById(
        @ApiParam(value = "Book ID provided by google books") @PathVariable bookId: String,
        @ApiParam(value = "Review ID") @PathVariable reviewId: String
    ): ReviewResponse =
        ReviewResponse(
            "読書 太郎",
            "2020年8月1日 13時00分",
